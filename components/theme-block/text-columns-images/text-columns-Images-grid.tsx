...
const TextColumnsImagesGrid = ({ data, variant = "default" }: any) => {
  const themeData = {
    text_alignment: {
      Left: "align-left",
  ...
  return (
    <section
      className={`text-columns-images text-columns-images--grid ${
        variant === gridCol2 ? "columns-images--grid-col-2" : ""
      }`}
    >
              ...
              return (
                <div className="text-columns-images__product" key={index}>
                  {item.data["tagline.show_image"] && (
                    <div className={`product__image ${getImageHeight()}`}>
...
export default TextColumnsImagesGrid;
